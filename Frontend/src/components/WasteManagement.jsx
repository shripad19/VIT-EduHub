...
export default function WasteManagement() {
  ...
  const editpost = async (blog, password) => {
    try {
      const response = await fetch("http://localhost:5000/wastemanagefetchpost", {
        method: "post",
      ...
      if (post && post.password === password) {
        navigate("/student/chem/waste-management/editpost", { state: blog });
      } else {
  ...
  const deletepost = async (id, password) => {
    try {
      const response = await fetch("http://localhost:5000/wastemanagedeletepost", {
        method: "post",
    ...
    <>
      ...
      <div className="f2fhomeparent">
        <div className="f2fnav">
          <h1 className="f2fhomeheader">Waste Management Blogs</h1>
        </div>

        <div className="maincontainer">
          <div className="newpostlinkblock">
            <Link className="newpostlink" to="/student/chem/waste-management/newpost">
              New post
            </Link>
          </div>
