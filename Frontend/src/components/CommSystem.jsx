...
export default function CommSystem() {
  ...
  const editpost = async (blog, password) => {
    try {
      const response = await fetch("http://localhost:5000/CSfetchpost", {
        method: "post",
      ...
      if (post && post.password === password) {
        navigate("/student/entc/communication-systems/editpost", { state: blog });
      } else {
  ...
  const deletepost = async (id, password) => {
    try {
      const response = await fetch("http://localhost:5000/CSdeletepost", {
        method: "post",
    ...
    <>
      ...
      <div className="f2fhomeparent">
        <div className="f2fnav">
          <h1 className="f2fhomeheader">Communication Systems Blogs</h1>
        </div>

        <div className="maincontainer">
          <div className="newpostlinkblock">
            <Link className="newpostlink" to="/student/entc/communication-systems/newpost">
              New post
            </Link>
          </div>
