...
export default function Thermodynamics() {
  ...
  const editpost = async (blog, password) => {
    try {
      const response = await fetch("http://localhost:5000/thermofetchpost", {
        method: "post",
      ...
      if (post && post.password === password) {
        navigate("/student/mech/thermodynamics/editpost", { state: blog });
      } else {
  ...
  const deletepost = async (id, password) => {
    try {
      const response = await fetch("http://localhost:5000/thermodeletepost", {
        method: "post",
    ...
    <>
      ...
      <div className="f2fhomeparent">
        <div className="f2fnav">
          <h1 className="f2fhomeheader">Thermodynamics Blogs</h1>
        </div>

        <div className="maincontainer">
          <div className="newpostlinkblock">
            <Link className="newpostlink" to="/student/mech/thermodynamics/newpost">
              New post
            </Link>
          </div>
