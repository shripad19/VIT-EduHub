...
export default function FluidMechanics() {
  ...
  const returnHome = () => {
    navigate("/student/mech");
  };

  const editpost = async (blog, password) => {
    try {
      const response = await fetch("http://localhost:5000/fluidmechfetchpost", {
        method: "post",
      ...
      if (post && post.password === password) {
        navigate("/student/mech/fluid-mechanics/editpost", { state: blog });
      } else {
  ...
  const deletepost = async (id, password) => {
    try {
      const response = await fetch("http://localhost:5000/fluidmechdeletepost", {
        method: "post",
    ...
    <>
      ...
      <div className="f2fhomeparent">
        <div className="f2fnav">
          <h1 className="f2fhomeheader">Fluid Mechanics Blogs</h1>
        </div>

        <div className="maincontainer">
          <div className="newpostlinkblock">
            <Link className="newpostlink" to="/student/mech/fluid-mechanics/newpost">
              New post
            </Link>
          </div>
