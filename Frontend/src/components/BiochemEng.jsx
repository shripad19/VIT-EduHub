...
export default function BiochemEng() {
  ...
  const returnHome = () => {
    navigate("/student/chem");
  };

  const editpost = async (blog, password) => {
    try {
      const response = await fetch("http://localhost:5000/biochemfetchpost", {
        method: "post",
      ...
      if (post && post.password === password) {
        navigate("/student/chem/biochemical-engineering/editpost", { state: blog });
      } else {
  ...
  const deletepost = async (id, password) => {
    try {
      const response = await fetch("http://localhost:5000/biochemdeletepost", {
        method: "post",
    ...
    <>
      ...
      <div className="f2fhomeparent">
        <div className="f2fnav">
          <h1 className="f2fhomeheader">Biochemical Engineering Blogs</h1>
        </div>

        <div className="maincontainer">
          <div className="newpostlinkblock">
            <Link className="newpostlink" to="/student/chem/biochemical-engineering/newpost">
              New post
            </Link>
          </div>
